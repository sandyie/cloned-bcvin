...
#thsi script Aril 15th 2020. Quadratic element added 20th APrill.  

#running a mixed linear model of bud winter hardiness regressed against air temperature. There is grouping 
#on the intercept and slope for variety, and a quaratic term.   
...
simTempsZ <- (simTemps - mean(simTemps))/sd(simTemps)
...
#quadratic term (chosen through trial and error to find one that looks ok)
betaQuad <- 0.25
...
simLTEVar <- alphaVarObs  + betaVarObs * simTempsZ + betaQuad * simTempsZ^2 + eps

#combine into a single data table

simVarData <- data.frame(cbind(simTempsZ, varNamesRep, simLTEVar))
str(simVarData)
simVarData[order(simVarData$varNames),]
simVarData$varNamesRep <- as.factor(simVarData$varNamesRep )

plot(simVarData$simLTEVar ~ simVarData$simTempsZ)
...
preTempsZ <- (preTemps - mean(preTemps))/sd(preTemps)
...
randomEffectsPre$beta_v <- NA 
randomEffectsPre$beta_quad <- NA 
...
#randomEffectsPre$x <- rep(preTempsZ, times = n_vars)
...
for (counter in 1:Ni){
	...
	#quadratic parameter
	randomEffectsPre$beta_quad[randomEffectsPre$rep == counter] <- beta_quad[counter]
...
fit6 <- stan(file = "stan/quad_slopeVarietyCov.stan", data = stan_data4, warmup = 1000, 
	iter = 2000, chains = 4, cores = 4, thin = 1, , control = list(max_treedepth = 15, adapt_delta = 0.97))
...
plot(density(post$beta_q))
# check some hyperparameters - dont understand how thsi is different from alpha_g and beta_g (grand alpha and grand beta).
#i checked these above in lines 221 and 223?
...
plot(realYs$lowerHPDI ~ simVarData$simTempsZ,col="green", type = "l")
lines(realYs$upperHPDI ~ simVarData$simTempsZ,col="green")
lines(realYs$meanRealY ~ simVarData$simTempsZ,col="black")
