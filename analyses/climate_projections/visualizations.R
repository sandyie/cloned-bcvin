...
variable_list <- list.files("D:/climate_projection_data/bcvin_raster/anomaly", pattern = ".asc")
setwd("D:/climate_projection_data/bcvin_raster/anomaly")
...
combined_list <- list.files("D:/climate_projection_data/bcvin_raster/combined", pattern = c(".asc"))
setwd("D:/climate_projection_data/bcvin_raster/combined")
...
loadLeaflet <- function(myDescriptiveVariable) {
  ...
  addLayersControl(baseGroups = c("Esri World Topographic", "Esri World Imagery"),
                   overlayGroups = c("Moderate Warming", "High Warming"))%>%
...
loadStatic <- function(myDescriptiveVariable){ #this doesn't work quite yet. Still need more info on whether or not to keep CV 
 ...
 #start anomalies graphs
  ...
  crs(stack2) <- sp::CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
...
#start 2 panel     
  ...
  assign(
    x = paste(myDescriptiveVariable, "_anomalies_v4", sep = ""),
  ...
  assign(#need to make an ifelse for ppt if we want to keep CV
    x = paste(myDescriptiveVariable, "_raw_v4", sep = ""),
    value = static_vals,
    envir = .GlobalEnv
...
setwd("D:/climate_projection_data/bcvin_raster")
for(i in 1:length(leafletList)){
  saveWidget(get(paste(leafletList[i])), paste(leafletList[i], "_v4.html", sep = ""), selfcontained = TRUE)
}

lapply(leafletList, FUN = loadStatic)
GDD5_05_10_anomalies_v4
GDD5_05_10_raw_v4
GDD5_04_09_anomalies_v4
GDD5_04_09_raw_v4
tmin_09_anomalies_v4
tmin_09_raw_v4
tmin_10_anomalies_v4
tmin_10_raw_v4
ppt_09_anomalies_v4
ppt_09_raw_v4
ppt_10_anomalies_v4
ppt_10_raw_v4
tmax_06_anomalies_v4
tmax_06_raw_v4
tmax_07_anomalies_v4
tmax_07_raw_v4
tmax_08_anomalies_v4
tmax_08_raw_v4
tmax_09_anomalies_v4
tmax_09_raw_v4
tmin_03_anomalies_v4
tmin_03_raw_v4
tmin_04_anomalies_v4
tmin_04_raw_v4
minTemp_12_03_anomalies_v4
minTemp_12_03_raw_v4
