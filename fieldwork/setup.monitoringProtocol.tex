...
So, you skip sampling when all the main crop of berries are destroyed by birds, are all diseased or shriveled or are oddly undeveloped (%e.g., see Table 2 in Chicken and hen). But when else do you stop measuring Brix? Two options:
\begin{enumerate}
...
{\bf Shipping grapes to UBC} %What are the shipping instructions?
...
\begin{smitemize}
\item refractometer %add brand and make
...
\item Use a pipette (with a disposable tip) to add juice on the prism, covering the prism. % In the past we have used plastic spoons, which we rinsed and wiped with Kimwipes between each use. 
